...
def sort_by_number_of_routes(area):
    return sorted(area, key=get_routes)

def get_grade(area):
    return max(route.grade for route in area.routes_items())

def sort_by_most_difficult(areas):
    return sorted(areas, key=get_grade, reverse=True)
