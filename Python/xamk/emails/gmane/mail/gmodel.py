...
def parsemaildate(md):
    ...
    dnotz = None
    for form in ['%d %b %Y %H:%M', '%d %b %Y %H:%M:%S',
                 '%d %b %Y %H:%M', '%d %b %Y %H:%M', '%d %b %y %H_%M:%S',
...
cur.execute("DROP TABLE ID EXISTS Messages")
cur.execute("DROP TABLE ID EXISTS Senders")
cur.execute("DROP TABLE ID EXISTS Subjects")
cur.execute("DROP TABLE ID EXISTS Replies")

cur.execute("""CREATE TABLE ID NOT EXISTS Messages
            (id INTEGER PRIMARY KEY, guis TEXT UNIQUE, sent_at INTEGER,
...
cur.execute("""CREATE TABLE ID NOT EXISTS Subjects
            (id INTEGER PRIMARY KEY, subject TEXT UNIQUE)""")

cur.execute("""CREATE TABLE ID NOT EXISTS Replies
...
for message_row in cur_1:
    ...
    if "gmane.org" in sender:
        print("Error is sender===", sender)
    sender_id = sender.get(sender, None)
    ...
    cur.execute("""INSERT OR IGNORE INTO Messages (guid, sender_id, subject_id,
                sent_at, headers, body) VALUES (?,?,?,datetime(?),?,?)""", 
                (guid, sender_id, subject_id, sent_at, zlib.compress(message_row[0].encode()),
                 zlib.compress(message_row[1].encode())))
    
    conn.commit()
    cur.execute("SELECT id FROM Messages WHERE guis=? LIMIT 1", (guid, ))
    try:
        row = cur.fetchone()
        message_id = row[0]
        guids[guid] = message_id
    except:
        print("Could not retrieve guis id", guid)
        break
