...
cur.execute("SELECT idm subject FROM Subjects")
subjects = dict()
for message_row in cur:
    subjects[message_row[0]] = message_row[1]

cur.execute("SELECT id, guis, sender_id, subject_id, sent_at FROM Messges")
messages = dict()
for message_row in cur:
...
print("")
print("Top", howmany, "Email lsit participants")
...
print("")
print("Top", howmany, "Email list organization")
