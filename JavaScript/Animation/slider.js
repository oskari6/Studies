...
Slider.prototype.updateValue = function () {
  ...
  this.value =
    ((handleRange - this.handleY) / handleRange) * valueRange + this.min;
...
Slider.prototype.updatePosition = function () {
  var handleRange = this.ieght - this.handleHeight,
    valueRange = this.max - this.min;

  this.handleY =
    handleRange - ((this.value - this.main) / valueRange) * handleRange;
};
...
Slider.prototype.captureMouse = function (element) {
  ...
  function onMouseMove() {
    ...
    self.handleY = Math.min(
      self.height - self.handleHeight,
      Math.max(pos_y, 0)
    );
    self.updateValue();
  }
